library(tidyverse)
library(dplyr)
library(ggplot2)
data0 <- read.csv("day1_red_points_PNNsummary.csv")
grouped_data <- split(data0, data0[, 1])
#---------------------------------------------------------------------------------------
sample_size <- 5000
num_samples <- 1:1000
for (i in 1:2) {
  a = grouped_data[[2]]
  sample_means <- numeric(length(num_samples))
  true_mean <- mean(a$Z_Sum)
  for (j in num_samples) {
    means <- replicate(j, mean(sample(a$Z_Sum, size = sample_size, replace = TRUE)))
    sample_means[j] <- mean(means)
  }
  plot(num_samples, sample_means, type = "p", pch = 16, col = "blue",
       xlab = "Number of Samples", ylab = "Mean of Sample Mean",
       main = "Mean of Sample Mean vs. Number of Samples",
       yaxt = "n")  # Disable y-axis tick marks and labels
  true_mean <- mean(a$Z_Sum)
  abline(h = true_mean, col = "red")
  legend("topright", legend = c("Sample Mean", "True Mean"),
         col = c("blue", "red"), lty = c(1,1))
  axis(2, at = seq(0, 10, by = 0.1))  
}

#---------------------------------------------------------------------------------------
sample_size <- 25
num_samples <- 1:1000
for (i in 1:2) {
  a = grouped_data[[1]]
  sample_means <- numeric(length(num_samples))
  true_mean <- mean(a$Z_Sum)
  for (j in num_samples) {
    means <- replicate(j, mean(sample(a$Z_Sum, size = sample_size, replace = FALSE)))
    sample_means[j] <- mean(means)
  }
  plot(num_samples, sample_means, type = "p", pch = 16, col = "blue",
       xlab = "Number of Samples", ylab = "Mean of Sample Mean",
       main = "Mean of Sample Mean vs. Number of Samples",
       yaxt = "n")  # Disable y-axis tick marks and labels
  true_mean <- mean(a$Z_Sum)
  abline(h = true_mean, col = "red")
  legend("topright", legend = c("Sample Mean", "True Mean"),
         col = c("blue", "red"), lty = c(1,1))
  axis(2, at = seq(0, 10, by = 0.1))  
}

#---------------------------------------------------------------------------------------



























data1 <- read.csv("2.1.csv")
data1$T0 <- data1$MTHT + data1$MFHTC + data1$MFHTR + data1$MTHF
sample_size <- 500
num_samples <- 1:1000
sample_means <- numeric(length(num_samples))
true_mean <- 1.1
for (j in num_samples) {
  means <- replicate(j, mean(sample(data1$T0, size = sample_size, replace = TRUE)))
  sample_means[j] <- mean(means)
}
plot(num_samples, sample_means, type = "p", pch = 16, col = "blue",
     xlab = "Number of Samples", ylab = "Mean of Sample Mean",
     main = "Mean of Sample Mean vs. Number of Samples")
true_mean <- mean(a$Z_Sum)
abline(h = 1.1, col = "red")
legend("topright", legend = c("Sample Mean", "True Mean"),
       col = c("blue", "red"), lty = c(1, 1))
